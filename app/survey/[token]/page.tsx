...
export default async function SurveyPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!loaded) {
    return <Frame>
      <h1>설문을 찾을 수 없습니다</h1>
      <p className="survey-note">주소가 잘못되었거나 설문이 삭제되었습니다. 메일에 있는 링크를 다시 확인해 주세요.</p>
    </Frame>;
  }
  ...
  if (survey.answered) {
    return <Frame>
      ...
      <p className="survey-note">이미 제출하신 설문입니다. 남겨 주신 의견은 다음 교육 설계에 반영됩니다.</p>
    </Frame>;
  }

  if (survey.status !== "open") {
    return <Frame>
      <h1>{survey.status === "closed" ? "마감된 설문입니다" : "아직 열리지 않은 설문입니다"}</h1>
      <p className="survey-note">{subtitle}</p>
