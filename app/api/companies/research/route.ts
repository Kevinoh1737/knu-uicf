...
export async function POST(request: Request) {
  try {
    ...
    const generated = await generateWithGemini({ role: "companyResearch", prompt: `${prompt}\n\n출력 문체: 화면에 바로 표시할 짧은 명사형·구문 중심으로 작성하세요. '~합니다', '~습니다', '~됩니다', '~한다', '~된다', '~이다'로 끝나는 서술형 문장은 피하고, 중복 설명은 제외하세요. comparisonTags는 업종, 제품 유형, 고객 유형, 업무 방식 기준의 표준화된 짧은 태그 4~6개로 작성하세요.`, responseSchema, temperature: 0.15 });
    ...
    try {
      const questionnaire = await generateWithGemini({ role: "questionnaireDesign", prompt: questionnairePrompt, responseSchema: questionnaireSchema, temperature: 0.15 });
...
${draftTailoredQuestions.map((question, index) => `${index + 1}. ${question}`).join("\n")}`;
      const review = await generateWithGemini({ role: "questionnaireDesign", prompt: questionnaireReviewPrompt, responseSchema: questionnaireReviewSchema, temperature: 0.05 });
