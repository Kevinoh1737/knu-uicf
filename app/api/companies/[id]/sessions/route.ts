...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    /**
     * 서로 기다릴 이유가 없는 것은 함께 부른다. 예전에는 일곱 번을 줄 세워 기다렸고
     * (실측 합계 약 580ms), 그중 뒤의 넷은 앞의 답을 쓰지도 않았다. 강의 목록만 먼저
     * 알면 되는 것이 셋(계약·수강·설문지)이라 물결 셋으로 접힌다.
     */
    const [
      { data: sessions, error },
      { data: company },
      ...
      // 단계 표시는 기업 전체의 진행이라 상담·질문지도 함께 읽는다. research 는 '있는가'만
      // 보면 되므로 통째로 끌어오지 않는다 — 조사 보고서 전문이 들어 있는 칸이다.
      supabase.from("company_research").select("questions,stage,research").eq("id", id).single(),
      supabase.from("company_consultations").select("id", { count: "exact", head: true })
        .eq("company_id", id).eq("status", "completed"),
      // 강사를 배정할 때 고르는 목록. 화면마다 따로 부르지 않도록 함께 내려보낸다.
      supabase.from("instructors").select("id,name,affiliation,job_title").eq("status", "active").order("name"),
    ]);
    if (error) throw error;

    const sessionIds = (sessions || []).map((session) => session.id as string);
    const [{ data: contracts }, { data: enrollments }, { data: surveys }] = sessionIds.length
      ? await Promise.all([
          ...
          supabase.from("session_learners").select("course_session_id,status").in("course_session_id", sessionIds),
          // 만족도. 발송은 이 화면에서 하고 결과 요약도 카드에 붙는다.
          supabase.from("surveys").select("id,course_session_id,title,status,questions").in("course_session_id", sessionIds),
        ])
      : [{ data: [] }, { data: [] }, { data: [] }];
